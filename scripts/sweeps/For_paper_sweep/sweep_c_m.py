...
from scripts.sweeps.nondimensional.ODEs.c_m import *
...
if __name__ == "__main__":
    ...
    wavelengths = np.empty(len(c_m_vals), dtype=object)
    frequencies = np.empty(len(c_m_vals), dtype=object)
    velocities = np.empty(len(c_m_vals), dtype=object)

    max_curvatures = np.empty(len(c_m_vals), dtype=object)

    for i, A in enumerate(c_m_vals):
        print("c_m: ", A)
        print(f'{i+1} out of {len(c_m_vals)}: {np.round((i)/len(c_m_vals)*100, 2)}% done')
    ...
    plt.figure()
    plt.plot(c_m_vals, wavelengths, 'ko')
    plt.xlabel("Peak muscular force " + r'$c_m$')
    plt.ylabel("Normalised wavelength " + r'$\lambda / L$')
    plt.title("Wavelength against peak muscular force")
    plt.savefig(
    ...
    plt.figure()
    plt.plot(c_m_vals, frequencies, 'ko')
    plt.xlabel("Peak muscular force " + r'$c_m$')
    plt.ylabel(r'$\text{Frequency} \, \mathrm{Hz}$')
    plt.title("Frequency against peak muscular force")
    plt.savefig(
    ...
    plt.figure()
    plt.plot(c_m_vals, velocities,'ko')
    plt.xlabel("Peak muscular force " + r'$c_m$')
    plt.ylabel("velocity " + r'$\mathrm{mm/s}$')
    plt.title("Velocity against peak muscular force")
    plt.savefig(
    ...
    plt.figure()
    plt.plot(c_m_vals, max_curvatures, 'ko')
    plt.xlabel("Peak muscular force " + r'$c_m$')
    plt.ylabel("Curvature amplitude " + r'$\mathrm{mm^{-1}}$')
    plt.title("Curvature amplitude against peak muscular force")
    plt.savefig(
